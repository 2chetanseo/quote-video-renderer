...
function animatedLine({ text, appear, settleY, fontSize, color }) {
  ...
  // Commas inside if() must be escaped as \, so ffmpeg does not treat them as filter separators.
  const alpha = `if(lt(t\\,${appear})\\,0\\,if(lt(t\\,${a2})\\,(t-${appear})/${dur}\\,1))`;
  // y slides from settleY+rise up to settleY over the same window, then holds.
  const y = `if(lt(t\\,${appear})\\,${settleY + rise}\\,if(lt(t\\,${a2})\\,${settleY + rise}-(${rise}*(t-${appear})/${dur})\\,${settleY}))`;
  return [
